...
public func configure(_ config: inout Config, env: inout Environment, services: inout Services) throws {
    ...
    let dbConfig = PostgreSQLDatabaseConfig(hostname: dbHostname, port: dbPort, username: dbUsername, database: db, password: dbPassword, transport: PostgreSQLConnection.TransportConfig.unverifiedTLS)
    let dbConfig = Post
